...
As with the arithmetic indices, different geometric indices correspond to different choices for the weights.\footnote{It is interesting to note that the weights can also be chosen so that any geometric index is also an arithmetic index (and vice versa). That is, for any \(\omega_{i}\), there is an \(\tilde{\omega}_{i}\) such that \(\prod_{i = 1}^{n} (p_{i1} / p_{i0})^{\omega_{i}} = \sum_{i = 1}^{n} \tilde{\omega}_{i} p_{i1} / p_{i0}\). See Balk (2008, sec. 4.2) for what these weights look like. Framing an index as arithmetic or geometric is really just a case of framing the weights. The same holds true for a number of other types of indices, such as harmonic indices---see Martin (2020).}
...
\FunctionTok{c}\NormalTok{(}\AttributeTok{Carli =} \FunctionTok{arithmetic\_mean}\NormalTok{(relatives), }
  ...
  \AttributeTok{Arithmetic =} \FunctionTok{arithmetic\_mean}\NormalTok{(relatives, weights),}
  \AttributeTok{Geometric =} \FunctionTok{geometric\_mean}\NormalTok{(relatives, weights))}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{c}\NormalTok{(}\AttributeTok{Laspeyres =} \FunctionTok{arithmetic\_mean}\NormalTok{(relatives, share0),}
  ...
  \StringTok{\textasciigrave{}}\AttributeTok{Geometric Laspeyres}\StringTok{\textasciigrave{}} \OtherTok{=} \FunctionTok{geometric\_mean}\NormalTok{(relatives, share0),}
  \StringTok{\textasciigrave{}}\AttributeTok{Geometric Paasche}\StringTok{\textasciigrave{}} \OtherTok{=} \FunctionTok{geometric\_mean}\NormalTok{(relatives, share1),}
...
\NormalTok{index\_top }\OtherTok{\textless{}{-}} \FunctionTok{geometric\_mean}\NormalTok{(index\_ab, }\FunctionTok{tapply}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{weight, dat}\SpecialCharTok{$}\NormalTok{group, sum))}
...
It is worth remarking that the Fisher index usually comes out as the theoretical ideal in an axiomatic framework.\footnote{Or sometimes the Törnqvist or Walsh indices, depending on the particular axioms---see Balk and Diewert (2001) and Balk (2008, sec. 3.6.4 and 3.6.5.).} This occurs because the product test is maintained without requiring consistency in aggregation, as well as imposing some other tests. Consequently, the Fisher index may be more appropriate if a price index is not calculated using a hierarchical structure. But it is important to note that this is not a foregone conclusion---one of the key results from the axiomatic approach is the ideal price index depends on its purpose, and there do not exist any universally applicable price indices.
...
In many cases the Fisher or Törnqvist index offers an even better approximation to an input-price index than the Laspeyres or geometric Laspeyres index. The argument for why these indices can be a better approximation are more complex---see ILO et al. (2004a, chap. 17) and ILO et al. (2004b, chap. 17) for details. The cost for this better approximation, however, is the need for more information about period \(t\) quantities, which may or may not be available at the time when the index is actually calculated.\footnote{In certain cases these approximations are exact. A Laspeyres index will be a true input-price index if the firm has a Leontief production function; a Fisher index will also be exact in this case. Similarly, a geometric Laspeyres will be a true input-price index if the firm has a Cobb-Douglas production functions; a Törnqvist index will also be exact in this case. The Lloyd-Moulton price index is an interesting case as it is exact whenever the firm has a constant elasticity of substitution production function, but it only requires period-0 quantity information.}
...
\end{align*}
Different arithmetic price-index formulas---e.g., Laspeyres, Paasche, Lowe---thus correspond to different statements about what determines the probability of observing a transaction. For example, with a Laspeyres index the probabilities are given by period-0 expenditure or revenue shares. This amounts to a statement that the probability of observing a transaction for a good is the likelihood of spending or receiving a dollar on that good in period 0.\footnote{Selvanathan and Rao (1994, chap. 3) give an alternate interpretation: different index-number formulas come from different statements about the variance structure of price relatives and the use of inverse-variance weights to produce efficient estimators of inflation.}
...
\midrule
\endhead
gps\_prices & Monthly price quotes for widget types ``A'' to ``J'' from January 2018 to December 2019 from the Globular Prices System. \\
micro\_prices & Publicly available microdata for daily transactions of type ``K'' widgets from January 2018 to December 2019. \\
...
\def\labelenumi{\arabic{enumi}.}
\item
  Consult your monthly, fixed-based indices. Which province saw the largest increase in prices since the beginning of 2018? By how much did prices change? Which province saw the smallest increase in prices since the beginning of 2018? By how much did prices change?
\item
  Convert you fixed-based index into a month-over-month index. What was the single largest month-over-month movement in prices at the provincial level? In which province and month did this occur? (Hint: You can use \texttt{piar}'s \texttt{unchain()} function to convert a fixed-base index into a month-over-month one.)
\item
  Convert your monthly fixed-base index into a quarterly index, and rebase it to Q1 2019. Which region had the highest prices in Q4 2019 as compared to Q1 2019? What was the value of the quarterly index there in that quarter? Which region had the the lowest prices in Q1 2018 as compared to Q1 2019? What was the value of the quarterly index there in that quarter? (Hint: you can use \texttt{piar}'s \texttt{mean()} method to convert a monthly index to a quarterly one.)
\item
...
These instructions outline a simple way to build the price index for widgets, although feel free to use a different approach if it's easier. The goal is to understand the process, rather than design the most computationally efficient system. In order to ensure that simply following these instructions isn't a trivial exercise, parts of the code from the snippets below have been omitted and replaced with tags in angle brackets. Wherever you see \texttt{\textless{}action\textgreater{}}, you should insert the name of a function. You can think of functions as ``verbs'' that perform actions on your data. Wherever you see \texttt{\textless{}variable\textgreater{}}, you should insert the name of one of your data structures. You can think of variables as ``nouns'' that you manipulate with ``verbs,'' or functions. Finally, wherever you see \texttt{\textless{}parameter\textgreater{}}, you should insert additional arguments to be passed to a function. You can think of these arguments or parameters as ``adverbs'' that modify your functions, or ``verbs.'' In order to help you out, some lines of code have hints next to them marked with \texttt{\#\ **HINT**}.
...
The \href{https://cran.r-project.org/package=gpindex}{gpindex}, and \href{https://github.com/marberts/piar}{piar} packages will be used in this module. Install them as follows if they aren't already installed, and then load them. (Hint: it is worth going through the documentation in the \texttt{gpindex} and \texttt{piar} packages.)

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{install.packages}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"gpindex"}\NormalTok{, }\StringTok{"piar"}\NormalTok{))}

\FunctionTok{library}\NormalTok{(gpindex)}
\FunctionTok{library}\NormalTok{(piar)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \textbf{Calculate the period-over-period elemental indices for product K}
\end{enumerate}

Using the microdata for product K in the \texttt{micro\_prices} file, calculate the geometric average of the prices in each province in each month in each reference year. Then, calculate the ratio of the geometric averages computed in step 3, making sure to have a price relative of 1 in January 2018 and January 2019. (January 2019 will be the link month when it is time to chain the index.)

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#{-}{-}{-}{-} Product \textquotesingle{}K\textquotesingle{} index {-}{-}{-}{-}}
\CommentTok{\# Turn period into a year{-}month and combine product and province into a }
\CommentTok{\# factor for elemental aggregates}
\NormalTok{micro\_prices }\OtherTok{\textless{}{-}} \FunctionTok{transform}\NormalTok{(micro\_prices, }
                          \AttributeTok{period =} \FunctionTok{format}\NormalTok{(period, }\StringTok{"\%Y\%m"}\NormalTok{), }
                          \AttributeTok{ea =} \FunctionTok{interaction}\NormalTok{(province, product, }\AttributeTok{sep =} \StringTok{""}\NormalTok{))}

\CommentTok{\# Get a single price for each time period by taking the geomean}
\CommentTok{\# **HINT** Take a look at the documentation for the aggregate methods }
\CommentTok{\# in the stats package}
\NormalTok{micro\_prices }\OtherTok{\textless{}{-}} \FunctionTok{aggregate}\NormalTok{(price }\SpecialCharTok{\textasciitilde{}} \ErrorTok{\textless{}}\NormalTok{variable}\SpecialCharTok{\textgreater{}} \SpecialCharTok{+} \ErrorTok{\textless{}}\NormalTok{variable}\SpecialCharTok{\textgreater{}} \SpecialCharTok{+} \ErrorTok{\textless{}}\NormalTok{variable}\SpecialCharTok{\textgreater{}}\NormalTok{,}
\NormalTok{                          micro\_prices,}
                          \SpecialCharTok{\textless{}}\NormalTok{action}\SpecialCharTok{\textgreater{}}\NormalTok{)}

\CommentTok{\# Calculate elemental indexes for product \textquotesingle{}K\textquotesingle{}}
\CommentTok{\# **HINT** piar has a function for computing ratios of prices in consecutive months}
\NormalTok{index\_k }\OtherTok{\textless{}{-}} \FunctionTok{lapply}\NormalTok{(}\FunctionTok{split}\NormalTok{(micro\_prices, micro\_prices}\SpecialCharTok{$}\NormalTok{basket), }
                  \ControlFlowTok{function}\NormalTok{(df) \{}
                    \FunctionTok{with}\NormalTok{(df, }\FunctionTok{elemental\_index}\NormalTok{(}\SpecialCharTok{\textless{}}\NormalTok{action}\SpecialCharTok{\textgreater{}}\NormalTok{(price, period, ea), period, ea))}
\NormalTok{                  \})}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \textbf{Calculate the period-over-period elemental indices for products A through J, and merge them with them with the product K elemental indices}
\end{enumerate}

Using the data from the Globular Prices System in \texttt{gps\_prices}, calculate price relatives for each quote in each province in each month in each reference year and use them to compute elemental indices. Then, merge these indices with the product K indices so that they are all in the same tables. This should give you two tables of elemental indices for products A through K, one for each reference year.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#{-}{-}{-}{-} Product \textquotesingle{}A\textquotesingle{} to \textquotesingle{}J\textquotesingle{} indices {-}{-}{-}{-}}
\CommentTok{\# Melt into a long format}
\NormalTok{gps\_prices }\OtherTok{\textless{}{-}} \FunctionTok{reshape}\NormalTok{(gps\_prices, }
                      \AttributeTok{direction =} \SpecialCharTok{\textless{}}\NormalTok{parameter}\SpecialCharTok{\textgreater{}}\NormalTok{, }
                      \AttributeTok{varying =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"price"}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{3}\NormalTok{),}
                      \AttributeTok{v.names =} \StringTok{"price"}\NormalTok{,}
                      \AttributeTok{timevar =} \StringTok{"quote"}\NormalTok{,}
                      \AttributeTok{idvar =} \FunctionTok{c}\NormalTok{(}\StringTok{"ea"}\NormalTok{, }\StringTok{"period"}\NormalTok{, }\StringTok{"basket"}\NormalTok{))}

\CommentTok{\# Combine quote id and ea labels to make unique product labels}
\NormalTok{gps\_prices}\SpecialCharTok{$}\NormalTok{product }\OtherTok{\textless{}{-}} \FunctionTok{paste0}\NormalTok{(gps\_prices}\SpecialCharTok{$}\NormalTok{ea, gps\_prices}\SpecialCharTok{$}\NormalTok{quote)}

\CommentTok{\# Calculate elemental indexes for product \textquotesingle{}A\textquotesingle{} to \textquotesingle{}J\textquotesingle{}}
\CommentTok{\# **HINT** What type of elemental indices have you been asked to compute? }
\CommentTok{\# What type of mean does that imply? What is the corresponding order of the generalized mean?}
\NormalTok{index\_aj }\OtherTok{\textless{}{-}} \FunctionTok{lapply}\NormalTok{(}\FunctionTok{split}\NormalTok{(gps\_prices, gps\_prices}\SpecialCharTok{$}\NormalTok{basket), }
                  \ControlFlowTok{function}\NormalTok{(df) \{}
                    \FunctionTok{with}\NormalTok{(df, }\FunctionTok{elemental\_index}\NormalTok{(}\SpecialCharTok{\textless{}}\NormalTok{action}\SpecialCharTok{\textgreater{}}\NormalTok{(price, period, product), }
\NormalTok{                                             period, ea, }\AttributeTok{na.rm =} \SpecialCharTok{\textless{}}\NormalTok{parameter}\SpecialCharTok{\textgreater{}}\NormalTok{, }\AttributeTok{r =} \SpecialCharTok{\textless{}}\NormalTok{parameter}\SpecialCharTok{\textgreater{}}\NormalTok{))}
\NormalTok{                  \})}

\CommentTok{\#{-}{-}{-}{-} Merge elemental indices {-}{-}{-}{-}}
\NormalTok{elementals }\OtherTok{\textless{}{-}} \FunctionTok{Map}\NormalTok{(merge, }\SpecialCharTok{\textless{}}\NormalTok{variable}\SpecialCharTok{\textgreater{}}\NormalTok{, }\SpecialCharTok{\textless{}}\NormalTok{variable}\SpecialCharTok{\textgreater{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \textbf{Build the aggregation structures}
\end{enumerate}

An aggregation structure tells R how to build up your price indices. It indicates how the elemental indices are aggregated to build provincial indices, how the provincial indices are aggregated build regional indices, and how the regional indices are aggregated to build the national index. It also indicates which weights are assigned to each elemental index in the aggregation. Create one aggregation structure for each year, for a total of two.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#{-}{-}{-}{-} Aggregation structure {-}{-}{-}{-}}
\NormalTok{pias }\OtherTok{\textless{}{-}} \FunctionTok{lapply}\NormalTok{(}\FunctionTok{split}\NormalTok{(weights, weights}\SpecialCharTok{$}\NormalTok{year), }
               \ControlFlowTok{function}\NormalTok{(df) \{}
                 \SpecialCharTok{\textless{}}\NormalTok{action}\SpecialCharTok{\textgreater{}}\NormalTok{(df[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{], df}\SpecialCharTok{$}\NormalTok{weight)}
\NormalTok{               \})}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \textbf{Aggregate and chain the index}
\end{enumerate}

Take the elemental indices you calculated in Steps 1 and 2 and the aggregation structures you created in Step 3, and use them to calculate the aggregate indices for provinces, regions, and the entire country. (There is no need to manually price-update the aggregation weights for each month, as \texttt{piar}'s \texttt{aggregate} function will do this automatically.) You should now have a collection of month-over-month indices. Chain the 2018 and 2019 indices together, using January 2019 as the link month, to get an index from January 2018 to December 2019, with January 2018 as the base period.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#{-}{-}{-}{-} Aggregate, chain, and quarter {-}{-}{-}{-}}
\NormalTok{index }\OtherTok{\textless{}{-}} \FunctionTok{Map}\NormalTok{(}\SpecialCharTok{\textless{}}\NormalTok{variable}\SpecialCharTok{\textgreater{}}\NormalTok{, }\SpecialCharTok{\textless{}}\NormalTok{variable}\SpecialCharTok{\textgreater{}}\NormalTok{, pias)}

\CommentTok{\# **HINT** What do you have to remove from the 2019 index in order to chain it }
\CommentTok{\# together with the 2018 index?}
\NormalTok{index }\OtherTok{\textless{}{-}} \FunctionTok{chain}\NormalTok{(}\FunctionTok{stack}\NormalTok{(index[[}\StringTok{"2018"}\NormalTok{]][}\DecValTok{1}\SpecialCharTok{:}\DecValTok{16}\NormalTok{, ], index[[}\StringTok{"2019"}\NormalTok{]][}\DecValTok{1}\SpecialCharTok{:}\DecValTok{16}\NormalTok{, }\SpecialCharTok{\textless{}}\NormalTok{parameter}\SpecialCharTok{\textgreater{}}\NormalTok{]))}
\end{Highlighting}
\end{Shaded}
...
\leavevmode\vadjust pre{\hypertarget{ref-balk1995}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-cpimanual}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-ppimanual}{}}%
---------. 2004b. \emph{Producer Price Index Manual: Theory and Practice}. International Monetary Fund.

\leavevmode\vadjust pre{\hypertarget{ref-rppihandbook}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kirbymcgregor2019}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kirman1992}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-mcfadden1978}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-pollak1980}{}}%
Pollak, R. A. 1980. {``Group Cost-of-Living Indexes.''} \emph{The American Economic Review} 70 (2): 273--78.

\leavevmode\vadjust pre{\hypertarget{ref-rosen1974}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-selvanathan1994}{}}%
Selvanathan, E. A., and D. S. P. Rao. 1994. \emph{Index Numbers: A Stochastic Approach}. University of Michigan Press.
