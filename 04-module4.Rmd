...
2. Convert you fixed-based index into a month-over-month index. What was the single largest month-over-month movement in prices at the provincial level? In which province and month did this occur? (Hint: You can use `piar`'s `unchain()` function to convert a fixed-base index into a month-over-month one.)
