...
These instructions outline a simple way to build the price index for widgets, although feel free to use a different approach if it's easier. The goal is to understand the process, rather than design the most computationally efficient system. (Hint: look into the [dplyr](https://cran.r-project.org/package=dplyr) package, the [gpindex](https://cran.r-project.org/package=gpindex) package, and the [calpr](https://github.com/ppd-dpp/calpr) packages for some useful tools.)
...
1. **Make the weights**
    ...
    Using the `weights` file, make one set of weights that give the value share of each product sold within each province in each year, and another that gives the value share of all products sold in each province in each year. This first set of weights will be used to aggregate elemental indices to get 10 province-level indices, and the second set of weights will be used to aggregate the province-level indices to a national index. There should be 200 weights in the first file and 20 weights in the second file.
...
2. **Calculate the geomean for product K**
    ...
    ```{r, eval = FALSE}
dat_micro <- dat_micro %>%
  mutate(period = year_month(period)) %>%
  # your code here %>%
  summarize(geomean = mean_geometric(price))
    ```
...
dat_index <- dat_gps %>%
  # your code here %>%
  gather(key = 'quote', value = 'price', price1:price3) %>% # gather is from the tidyr package
...
5. **Price update the product weights**
    ...
    ```{r, eval = FALSE}
dat_index <- dat_index %>%  
  left_join(weights_prod, by = c('province', 'product', 'year')) %>%
  # your code here
    ```
...
my_index <- index_chain %>%
  ...
  mutate(index = index/index[period == '2019-01-01'] * 100)
